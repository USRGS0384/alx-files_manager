...
// Function to delete all but the newest file
async function cleanupOldFiles(folderPath) {
  try {
    ...
    sortedFiles.sort((a, b) => b.mtime - a.mtime); // Sort newest first

    // Keep only the latest file and delete the rest
    const filesToDelete = sortedFiles.slice(1); // All except the first (latest)
    for (const file of filesToDelete) {
      await fs.unlink(path.join(folderPath, file.file));
    }
  } catch (error) {
